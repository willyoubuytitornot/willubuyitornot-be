...
|----------|-----------|------------|
| 컨트롤러 HTTP 동작·응답 shape·상태코드 (슬라이스, 서비스 모킹) | `@WebMvcTest(XController.class)` + `MockMvc` + `@MockBean` 서비스 | ❌ |
| 컨트롤러→서비스→리포지토리 전체 경로 (실제 Mongo) | `@SpringBootTest(webEnvironment=MOCK)` + `@AutoConfigureMockMvc` + `AbstractMongoIntegrationTest`(Testcontainers) | ✅ |
| 서비스 비즈니스 로직 | 순수 단위 테스트(Mockito로 리포지토리 목) 또는 `@SpringBootTest` | ❌(목) |
| 리포지토리 쿼리·인덱스/unique 제약 | `@DataMongoTest` | ✅ |
| 전체 와이어링/통합 | `@SpringBootTest` | ✅ |

**우선순위:** 컨트롤러 슬라이스만 볼 때는 `@WebMvcTest`(컨트롤러) + Mockito(서비스). 서비스/리포지토리까지
실제 동작을 검증하려면 `com.willu.buyitornot.support.AbstractMongoIntegrationTest`를 상속해 Testcontainers로
띄운 실제 MongoDB에 연결한다(`UserControllerIntegrationTest` 참조). Docker가 없는 환경이면 컨테이너 기동이 실패하므로
미검증 항목으로 명시적으로 기록한다.
